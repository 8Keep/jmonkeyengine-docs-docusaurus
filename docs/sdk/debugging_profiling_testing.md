...
1. **LMB**  click OK. The jMonkeyEngine SDK creates a JUnit test skeleton in the Test Package directory.
1. The body of each generated test method is provided solely as a guide. In their place, you need to write your actual test cases!
1. You can use tests such as `assertTrue(), assertFalse(), assertEquals()`, or `assert()`.
  - The following example assertions test an addition method: `assert( add(1, 1) == 2); assertTrue( add(7,-5) == add(-5,7) )…`

1. "`Ideally`", you write a test case for every method (100% coverage).
...
1. Check the Test window to see successful tests (green) and failures (red).
1. If a test fails that has succeeded before, you know that your latest changes broke something!
...
- [Writing JUnit Tests](http://netbeans.org/kb/docs/java/junit-intro.html)
...
1. Choose "Debug &gt; Debug Main Project" to start a debugger session for the whole project. Or, **RMB** click a file and select "`Debug File`" to debug only one file.
1. The application starts running normally. If you have set a breakpoint, the execution stops in this line. Debugger windows open and print debugger output.
1. You can do many things now to track down a bug:
...
1. To stop debugging, choose "Debug &gt; End Debugger Session" from the menu.
...
1. Find the "`Java Profiler`" plugin ("`Java SE`" category) and check the Install box.
1. Click the install button and follow the instructions.
1. When you start the profiler for the first time, you are prompted to run a calibration once. Click OK in the "`Profiler`" integration dialog to complete the installation process.

### Monitoring and Analyzing

1. Choose Profile Project from the Profile menu.
1. Select one of three tasks:
...
1. **LMB** click Run. Your application starts and runs normally.
1. Use the Profiling window to track and collect live profiling results while you application is running.
...
- [Introduction to Profiling Java Applications (netbeans.org)](http://netbeans.org/kb/docs/java/profiler-intro.html)
- [Using Profiling Points (netbeans.org)](http://netbeans.org/kb/docs/java/profiler-profilingpoints.html)
