...
- To handle proper gamma corrected output colors, Opengl expose an ARB extension that allows you to output a color in linear space and have the GPU automatically correct it : [https://www.opengl.org/registry/specs/ARB/framebuffer_sRGB.txt](https://www.opengl.org/registry/specs/ARB/framebuffer_sRGB.txt)
...
1. Disable rendered output correction : renderer.setMainFrameBufferSrgb(false); (for example in the simpleInit method of your SimpleApplication).
1. Use the GammaCorrectionFilter in a FilterPostProcessor, and set the proper gamma value on it (default is 2.2).
