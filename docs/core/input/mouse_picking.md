...
1. Keep mouse pointer invisible using `inputManager.setCursorVisible(false)`.
1. Map the `pick target` action to a MouseButtonTrigger.
1. Implement the action in the Listener.
...
1. Map the `pick target` action to a MouseButtonTrigger.
1. Make the mouse pointer visible using `inputManager.setCursorVisible(true)`.
1. Remap the inputs for camera rotation, or deactivate camera rotation.
1. Implement the action in the Listener.
