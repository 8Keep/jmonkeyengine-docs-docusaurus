...
![custom_mesh.png](/wiki-assets/docs/core/assets/images/scene/custom_mesh.png)
...
- Full code sample: [TestCustomMesh.java](https://github.com/jMonkeyEngine/jmonkeyengine/blob/master/jme3-examples/src/main/java/jme3test/model/shape/TestCustomMesh.java)

## Polygon Meshes

Polygon [mesh](mesh.md)es are made up of triangles. The corners of the triangles are called vertices. When ever you create any new shape, you break it down into triangles.
...
:::tip
If you get the order wrong (clockwise) for some of the triangles, then these triangles face backwards. If the [Spatial](spatial.md)'s material uses the default `FaceCullMode.Back` (see "`face`" culling), the broken triangles appear as holes in the rendered mesh. You need to identify and fix them in your code.
:::
...
1. We assign the data to the appropriate type of buffer inside the `Mesh` object. The three buffer types (`Position`, `TextCoord`, `Index`) are taken from an enum in `com.jme3.scene.VertexBuffer.Type`.
1. The integer parameter describes the number of components of the values. Vertex positions are 3 float values, texture coordinates are 2 float values, and the indices are 3 ints representing 3 vertices in a triangle.
1. To render the mesh in the scene, we need to pre-calculate the bounding volume of our new mesh: Call the `updateBound()` method on it.
...
1. Call `updateModelBound()` on the Geometry object containing the mesh - which in turns calls `updateBound()` on the mesh.
...
There are more vertex buffers in a Mesh than the three shown above. For an overview, see also [mesh](mesh.md).
...
- [Spatial](spatial.md) – contains more info about how to debug custom meshes (that do not render as expected) by changing the default culling behaviour.
