...
You want to make the most of your 3D models by specifying good looking material parameters. The developers must be in contact with the graphic designers regarding which of the [Material properties](materials_overview.md) they intend to use in their 3D models. You must have an understanding what [texture maps](../../tutorials/concepts/terminology.md#materials-textures) are, to be able to use texture-mapped materials.

:::important
Don't forget to add a [Light Source](../light/light_and_shadow.md) to the scene! All Materials (except "`Unshaded`" ones) are *invisible* without a light source.
:::

If you want more advanced background info: You can learn more about [Material Definitions](material_definitions.md) in general here. You can find the full list of Material Parameters in the [Material Definitions Properties](materials_overview.md) overview. The following sections introduce you to the most commonly used cases. You typically initialize Material objects in the `simpleInitApp()` method, and configure them using the setters described here. Then load the Materials using `myGeometry.setMaterial(mat)`.
...
This example creates a [Phong](http://en.wikipedia.org/wiki/Phong_reflection_model)-illuminated blue material. Use it for illuminated, naturalistic objects, such as characters, buildings, terrains, vehicles. Needs a light source, otherwise it will be invisible.
...
:::tip
Do you reuse Materials? You can [store Material properties in a .j3m file](../../sdk/material_editing.md) and load all properties in one line using
...
1. Specify at least Diffuse and Ambient colors. Set both to the same color in the standard case.
...
1. Specify the `NormalMap` texture for the Material.

```java
mat.setTexture("NormalMap", assetManager.loadTexture("Textures/wood_normal.png")); // with Lighting.j3md
```

[Learn more about creating and using NormalMaps and BumpMaps here.](http://en.wikipedia.org/wiki/Bump_mapping)
...
1. Specify the `Specular` and `Diffuse` colors of the shiny spot. +
...
1. (Optional) Specify a `SpecularMap` texture. +
...
1. Add one [BloomFilter PostProcessor](../effect/bloom_and_glow.md) in your simpleInitApp() method (only once, it is used by all glowing objects).
...
1. (Optional) Specify a `GlowMap` texture. +
...
Learn more about [Bloom and Glow](../effect/bloom_and_glow.md).
...
1. Specify BlendMode Alpha for the Material.

```java
mat.getAdditionalRenderState().setBlendMode(BlendMode.Alpha);
```

1. Put the Geometry (not the Material!) in the appropriate render queue bucket. +
...
1. (Optional) Specify other material settings.

<table>
  <thead>
    <tr>
      <th>Standard Material Transparency</th>
      <th>Description</th>
      <th>Example<br /></th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>getAdditionalRenderState().setBlendMode(BlendMode.Off);</td>
      <td>This is the default, no transparency.</td>
      <td>Use for all opaque objects like walls, floors, people…<br /></td>
    </tr>
    <tr>
      <td>getAdditionalRenderState().setBlendMode(BlendMode.Alpha);</td>
      <td>Interpolates the background pixel with the current pixel by using the current pixel's alpha.</td>
      <td>This is the most commonly used BlendMode for transparency and translucency: Frosted window panes, ice, glass, alpha-blended vegetation textures…<br /></td>
    </tr>
    <tr>
      <td>getAdditionalRenderState().setDepthWrite(false);</td>
      <td>Disables writing of the pixel's depth value to the depth buffer.</td>
      <td>Deactivate this on Materials if you expect two or more transparent/translucent objects to be obscuring one another, but you want to see through both.<br /></td>
    </tr>
    <tr>
      <td>getAdditionalRenderState().setAlphaTest(true) +<br />getAdditionalRenderState().setAlphaFallOff(0.5f);</td>
      <td>Enables Alpha Testing and uses an AlphaDiscardThreshold as alpha fall-off value. This means that gradients in the AlphaMap are no longer interpreted as soft translucency, but parts of the texture become either fully opaque or fully transparent. Only pixels above the alpha threshold (e.g. 0.5f) are rendered.</td>
      <td>Activate Alpha Testing for (partially) *transparent* objects such as foliage, hair, etc. +<br />Deactivate Alpha Testing for gradually *translucent* objects, such as colored glass, smoked glass, ghosts.<br /></td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    <tr>
      <td>getAdditionalRenderState().setWireframe(true);</td>
      <td>Switch to showing the (textured) Material in wireframe mode. The wireframe optionally uses the Material's `Color` value.</td>
      <td>Use wireframes to debug meshes, or for a "`matrix`" or "`holodeck`" effect.<br /></td>
    </tr>
    <tr>
      <td>getAdditionalRenderState().setLineWidth(2f);</td>
      <td>When in wireframe mode, sets the line width of the mesh.</td>
      <td></td>
    </tr>
  </tbody>
</table>
