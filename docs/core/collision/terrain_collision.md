...
This tutorial expands the HelloTerrain tutorial and makes the terrain solid. You combine what you learned in [Hello Terrain](../../tutorials/beginner/hello_terrain.md) and [Hello Collision](../../tutorials/beginner/hello_collision.md) and add a CollisionShape to the terrain. The terrain's CollisionShape lets the first-person player (who is also a CollisionShape) collide with the terrain, i.e. walk on it and stand on it.
...
Read [Hello Terrain](../../tutorials/beginner/hello_terrain.md) for details of the following parts that we reuse:

1. The `AbstractHeightMap` is an efficient way to describe the shape of the terrain.
1. The `Terrain.j3md`-based Material and its texture layers let you colorize rocky mountain, grassy valleys, and a paved path criss-crossing over the landscape.
1. The TerrainQuad is the finished `terrain` Spatial that you attach to the rootNode.

### The Collision Detection Code

Read [Hello Collision](../../tutorials/beginner/hello_collision.md) for details of the following parts that we reuse:

1. The `BulletAppState` lines activate physics.
1. The `ActionListener` (`onAction()`) lets you reconfigure the input handling for the first-person player, so it takes collision detection into account.
1. The custom `setUpKeys()` method loads your reconfigured input handlers. They now don't just walk blindly, but calculate the `walkDirection` vector that we need for collision detection.
1. `simpleUpdate()` uses the `walkDirection` vector and makes the character walk, while taking obstacles and solid walls/floor into account.
...
1. The physical first-person player is a CapsuleCollisionShape with a CharacterControl.
...
1. Add the control to the `terrain` to make it physical.
...
- [Hello Terrain](../../tutorials/beginner/hello_terrain.md),
