...
1. [jNavigationNative](https://github.com/QuietOne/jNavigation-native) contains Recast Navigation library and C++ wrapper for java
1. [jNavigation](https://github.com/QuietOne/jNavigation) is Java project that uses jNavigationNative and is the project that the end user will use
...
1. adding new methods for Recast use
...
1. Setting [ parameters for NetBeans compiler](https://netbeans.org/kb/docs/cnd/beginning-jni-linux.html)
1. Remove all headers from Header Files
1. Remove all source files *EXCEPT Recast_wrap.cxx* from Source Files
1. Right click on Header files, select `Add Existing Item…` or `Add Existing Items from Folders…` and select needed headers
1. Right click on Source files, select `Add Existing Item…` or `Add Existing Items from Folders…` and select needed source files
1. Build
1. Add built project to jNavigation project
1. Build jNavigation project
...
1. Install SWIG if not already
1. Open terminal and go to folder where the script is
1. Execute command `swig -c++ -java Recast.i`
1. Now SWIG will generate Java classes and new Recast_wrap.cxx
1. Recast_wrap.cxx put in jNavigationNative with new headers and source files, as previously mentioned
1. Build that project
1. For jNavigation side, put only new methods in RecastJNI, and use where they are being used. For that you can see in Java class that are build with SWIG.
1. If method uses some explicit SWIG type, try to use some method for converting it into jME type, or similar. You can probably find something in package `com.jme3.ai.navigation.utils`
