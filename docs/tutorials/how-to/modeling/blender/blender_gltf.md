...
Details of how to create models in Blender that are compatible with jME are given at [Creating assets in Blender3D](blender.md). Follow all details on that page for creating models before attempting an export.
...
As always, see [Best Practices](../../../concepts/best_practices.md) for additional information.

:::tip
[Blender Buffer Clearing](blender_buffer_clearing.md) should be performed prior to exporting any model from Blender.
:::
...
1. Blender supports lots of different types of shaders and complex materials with several different renderers. glTF supports a relatively basic set of PBR attributes. If the primary purpose of the materials is to produce glTF exported models use a simple `Principled BSDF` node in your materials with the following settings:
  ...
  1. Base color
  1. Diffuse and Normal (bump) textures
...
.youtube
![youtu.be/DZQ6HJgUheY](/wiki-assets/docs/tutorials/assets/images/how-to/modeling/blender/DZQ6HJgUheY.png)
...
1. `GLB`: a single file in binary format (the default)
1. `GLTF_EMBEDDED`: a single file in JSON format
1. `GLTF_SEPERATE`: multiple files for separate objects, textures etc.
...
1. There is no equivalent in glTF to Blender collections. These will not be represented in the imported structure.
1. If an object has no material, a default material will be created for it.
1. If an object has a single material, the corresponding node in the imported structure will have type `Geometry` with the object's mesh and the associated material.
1. If an object has more than one material, the corresponding node will have type `Node` with 1 child per material. Each of these children will be a `Geometry` with the associated material and a mesh containing the vertices assigned to the material.
...
If you use the jME3 SDK, you can explore the scene graph using the [Scene Explorer](../../../../sdk/scene_explorer.md).

See [Traverse the SceneGraph](../../../../core/scene/traverse_scenegraph.md) for assistance in finding the AnimControl of your models.

See [Animation in jME3](../../../../core/animation/animation.md) for further details on using animations in your code.

## Additional Reading

Thread announcing glTF support: [https://hub.jmonkeyengine.org/t/jme-gltf-support/39174](https://hub.jmonkeyengine.org/t/jme-gltf-support/39174)

Documentation for the python export functions: [https://docs.blender.org/api/current/bpy.ops.export_scene.html](https://docs.blender.org/api/current/bpy.ops.export_scene.html)

glTF specification: [https://github.com/KhronosGroup/glTF/tree/master/specification/2.0](https://github.com/KhronosGroup/glTF/tree/master/specification/2.0)
