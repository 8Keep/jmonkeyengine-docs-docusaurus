...
![beginner-terrain.png](/wiki-assets/docs/tutorials/assets/images/beginner/beginner-terrain.png)
...
![terrain-from-float-array.png](/wiki-assets/docs/tutorials/assets/images/beginner/terrain-from-float-array.png)
...
![terrain-from-heightmap.png](/wiki-assets/docs/tutorials/assets/images/beginner/terrain-from-heightmap.png)
...
:::tip
The jMonkeyEngine SDK comes with a [TerrainEditor plugin](../../sdk/terrain_editor.md). Using the TerrainEditor plugin, you can sculpt the terrain with the mouse, and save the result as heightmap. You can paint textures on the terrain and the plugin saves the resulting splat textures as alphamap(s). The following paragraphs describe the manual process for you.
:::
...
1. You '`paint`' three texture layers by using three colors: Red, blue and, green. You arbitrarily decide that…
  ...
  1. Green is dirt  – green is layer `Tex2`, so put the dirt  texture into Tex2.
  1. Blue  is roads – blue  is layer `Tex3`, so put the roads texture into Tex3.
...
1. Specify tile size: You want to terrain tiles of size 64x64, so you supply 64+1 = 65.
  - In general, 64 is a good starting value for terrain tiles.
1. Specify block size: Since you prepared a heightmap of size 512x512, you supply 512+1 = 513.
...
1. Supply the 512x512 heightmap object that you created.
...
1. Remember to attach the terrain to the rootNode.
...
1. If needed, scale and translate the terrain object, just like any other Spatial.
...
1. Change one parameter at a time, and the run the sample again. Note the differences. Can you find out which of the values has which effect on the generated terrain (look at the javadoc also)?
...
Can you combine what you learned here and in [Hello Collision](hello_collision.md), and [make the terrain solid](../../core/collision/terrain_collision.md)?
...
- [Advanced Terrain](../../core/terrain/terrain.md)
