...
This tutorial relies on what you have learned in the [Hello Input](hello_input_system.md) tutorial. You find more related code samples under [Mouse Picking](../../core/input/mouse_picking.md) and [Collision and Intersection](../../core/collision/collision_and_intersection.md).

![beginner-picking.png](/wiki-assets/docs/tutorials/assets/images/beginner/beginner-picking.png)
...
1. `makeFloor()` creates a gray floor node for "`target`" practice.
1. `initMark()` creates a red sphere (mark). We will use it later to mark the spot that was hit.
  - Note that the mark is not attached and therefor not visible at the start!

1. `initCrossHairs()` creates simple cross-hairs by printing a "`+`" sign in the middle of the screen.
...
1. Cast a ray from cam location into the cam direction.
1. Collect all intersections between the ray and `Shootable` nodes in the `results` list.
1. Use the results list to determine what was hit:
  1. For each hit, JME reports its distance from the camera, impact point, and the name of the mesh.
  1. Sort the results by distance.
  1. Take the closest result, it is the mesh that was hit.
...
1. The action casts a ray forward and determines intersections with shootable objects (= ray casting).
1. For any target that has been hit, it prints name, distance, and coordinates of the hit.
1. Finally it attaches a red mark to the closest result, to highlight the spot that was actually hit.
1. When nothing was hit, the results list is empty, and the red mark is removed.
...
:::tip
Notice how you use the provided method `results.getClosestCollision().getContactPoint()` to determine the _closest_ hit's location. If your game includes a "`weapon`" or "`spell`" that can hit multiple targets, you could also loop over the list of results, and interact with each of them.
:::
...
1. Reset result list.
1. Get 2D click coordinates.
1. Convert 2D screen coordinates to their 3D equivalent.
1. Aim the ray from the clicked 3D location forwards into the scene.
1. Collect intersections between ray and all nodes into a results list.
...
1. To change an object's color, you must first know its Geometry. Identify the node by identifying the target's name.
  - Use `Geometry g = closest.getGeometry();`

1. Create a new color material and set the node's Material to this color.
...
1. The inventory node is not attached to the rootNode.
