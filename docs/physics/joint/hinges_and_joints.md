...
The jMonkeyEngine3 has built-in support for [jBullet physics](http://jbullet.advel.cz) via the `com.jme3.bullet` package.
...
1. For the pendulum, we use a Spatial with a PhysicsControl, and we apply physical forces to them.
...
1. We can "`crank`" the handle and rotate the joint like a hinge, or we can let loose and expose the joints freely to gravity.
...
You'll understand that, when creating any type of joint, it is important to correctly specify the DOFs that the joint restricts, and the DOFs that the joint allows. For the typical DOF of a [ragDoll](../control/ragdoll.md) character's limbs, jME even offers a special joint, `ConeJoint`.
...
1. Specify the velocity with which the joint should rotate around the specified axis.
  - Use positive and negative numbers to change direction.

1. Specify the impulse for this motor. Heavier masses need a bigger impulse to be moved.
